  ...
  return (
    <>
      <section className="hero-grid overflow-hidden bg-primary text-white">
        <div className="section-shell grid min-h-[650px] items-center gap-12 py-14 lg:grid-cols-[1.06fr_.94fr] lg:py-20">
          ...
          <div className="relative mx-auto w-full max-w-[460px]">
            ...
            <div className="relative overflow-hidden rounded-[34px] border border-white/15 bg-white/10 p-3 shadow-2xl backdrop-blur">
              <MediaFrame media={HERO_MEDIA} eager className="aspect-[2/3] rounded-[25px] bg-white" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="section-shell section-pad">
        ...
        <div className="mt-16 grid gap-6 md:grid-cols-[1.25fr_.75fr] md:items-end">
          <div><p className="eyebrow text-primary">Популярные направления</p><h2 className="mt-3 max-w-4xl text-balance text-4xl font-bold leading-[1.04] tracking-[-.045em] md:text-6xl">Лечение, восстановление и эстетика</h2></div>
          <p className="text-base leading-7 text-on-surface-variant md:pb-1">Четыре направления, с которых пациенты чаще всего начинают знакомство с клиникой.</p>
        </div>
      ...
      <SplitSection tone="soft" media={
        <div className="relative grid grid-cols-2 gap-4">
          ...
          <div className="col-span-2 flex items-center gap-5 rounded-[24px] bg-primary p-5 text-white shadow-lg md:p-6">
            <img src="/images/brand/perfect-dental-mark.png" alt="" width="120" height="120" loading="lazy" className="size-14 shrink-0 rounded-2xl bg-white object-contain p-2" />
        ...
        <div className="mt-7"><FeatureGrid items={[
      ...
      <section className="section-shell section-pad">
        ...
        <div className="mt-10"><ProcessSteps items={['Консультация и знакомство', 'Диагностика по показаниям', 'План и стоимость лечения', 'Лечение и контроль результата']} /></div>
      </section>

      <SplitSection reverse media={
        <div className="relative">
          <MediaFrame media={DIAGNOSTICS_MEDIA} className="aspect-[16/10] rounded-[30px] bg-black shadow-xl" />
          <div className="absolute -bottom-5 -left-3 max-w-[250px] rounded-2xl border border-white/60 bg-white/95 p-5 shadow-xl backdrop-blur md:-left-7">
      ...
      <section className="section-shell section-pad">
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-[1.1fr_.9fr_.9fr]">
          <div className="relative flex min-h-[340px] flex-col justify-between overflow-hidden rounded-[28px] bg-primary p-7 text-white md:p-9">
            <Sparkles className="size-8 text-primary-fixed" aria-hidden="true" />
            <blockquote className="relative mt-16"><p className="text-balance text-2xl font-bold leading-9">«{REVIEWS[0].text}»</p><footer className="mt-6 text-sm text-white/70">{REVIEWS[0].author} · отзыв в 2GIS</footer></blockquote>
          </div>
          {REVIEWS.slice(1).map((review) => <TrustCard key={review.author} quote={review.text} author={review.author} href={review.sourceUrl} />)}
        </div>
      </section>
      <AppointmentCta />
    </>
