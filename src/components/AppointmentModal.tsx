...
import { CLINIC_INFO, DOCTORS, DOCTOR_PREVIEWS, SERVICES } from '../data/clinicData';
      ...
      <section ref={dialog} role="dialog" aria-modal="true" aria-labelledby={titleId} className="relative my-6 w-full max-w-xl overflow-hidden rounded-3xl bg-white shadow-2xl">
        ...
        {status === 'success' ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4 px-6 py-6 md:px-8" noValidate>
            ...
            <label className="block text-sm font-semibold">Врач или направление<select value={form.doctor} onChange={(e) => setForm({ ...form, doctor: e.target.value })} className="form-field mt-1.5"><option value="">Подберёт администратор</option><optgroup label="Врачи Perfect Dental">{DOCTOR_PREVIEWS.map((doctor) => <option key={doctor.id} value={doctor.name}>{doctor.name} — {doctor.specialty}</option>)}</optgroup><optgroup label="По направлению">{DOCTORS.map((doctor) => <option key={doctor.id} value={doctor.specialty}>{doctor.specialty}</option>)}</optgroup></select></label>
